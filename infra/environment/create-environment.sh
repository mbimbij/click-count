...
aws cloudformation package --template-file environment.yml --output-template environment-$ENVIRONMENT-packaged.yml --s3-bucket $CFN_S3_BUCKET_NAME
...
aws cloudformation deploy --stack-name $STACK_NAME --template-file environment-$ENVIRONMENT-packaged.yml --parameter-overrides file://$ENVIRONMENT-processed.json --capabilities CAPABILITY_NAMED_IAM
rm $ENVIRONMENT-processed.json
rm environment-$ENVIRONMENT-packaged.yml